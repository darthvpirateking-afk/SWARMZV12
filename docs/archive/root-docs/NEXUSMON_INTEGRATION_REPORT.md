...
### FILES NOT MODIFIED:
- `nexusmon_server.py` → Already complete, all wires present
...
### COMPILE CHECK:
```bash
$ python -m py_compile nexusmon_server.py nexusmon_organism.py
✅ PASS - No syntax errors
```

### LOCAL SERVER CHECK:
```bash
$ python -c "import nexusmon_server"
...
4. **Settings → Variables** (Required)
   - Confirm these environment variables are set:
     ...
     - `NEXUSMON_JWT_SECRET` → 32+ char secret for JWT signing
...
6. **Verify Deployment**
   ```bash
   # Test healthcheck
   curl https://nexusmonv10-production.up.railway.app/v1/health
   
   # Expected: {"ok":true,"service":"NEXUSMON API"}
   
   # Test organism cockpit
   curl https://nexusmonv10-production.up.railway.app/v1/nexusmon/organism/status
   
   # Should return full evolution state
   
   # Test cockpit UI
   curl https://nexusmonv10-production.up.railway.app/organism
