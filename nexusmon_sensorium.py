...
class Sensorium:
    ...
    def _process_signals(self, stats: Dict):
        # 1. Stress Evaluation
        if stats["cpu"] > 80.0:
            self.nerve.fire("SENSORIUM", "STRESS", payload={"cause": "CPU_PEAK", "val": stats["cpu"]}, intensity=2.0)
        
        if stats["mem"] > 85.0:
            self.nerve.fire("SENSORIUM", "STRESS", payload={"cause": "MEM_PEAK", "val": stats["mem"]}, intensity=1.5)

        # 2. Synergy Evaluation (Tranquility / Optimal Throughput)
        if stats["cpu"] < 20.0 and stats["mem"] < 50.0:
            self.nerve.fire("SENSORIUM", "SYNERGY", payload={"state": "TRANQUIL"}, intensity=0.5)

        if stats["net_recv"] > 1024 * 1024: # > 1MB of download
            self.nerve.fire("SENSORIUM", "SYNERGY", payload={"state": "DATA_ABSORPTION", "bits": stats["net_recv"]}, intensity=1.0)
