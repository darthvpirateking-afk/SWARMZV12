...
export const DEFAULT_PROGRESS: PlayerProgress = {
  ...
  partnerForm: "Seed",
  partnerXp: 0,
  army: [{ id: "shadow-1", role: "DPS", rarity: "Common", level: 1 }],
  chipLevel: 1,
  buildingLevel: 1,
  gateTier: 1,
  dataShards: 0,
...
function sanitizeArmy(army: unknown): ArmyUnit[] {
  if (!Array.isArray(army) || army.length === 0)
    return structuredClone(DEFAULT_PROGRESS.army);
      ...
      return {
        id:
          typeof unit.id === "string" && unit.id.length > 0
            ? unit.id
            : `unit-${Math.random().toString(16).slice(2, 8)}`,
        role: ROLES.includes(unit.role as ArmyRole)
          ? (unit.role as ArmyRole)
          : "DPS",
        rarity: RARITIES.includes(unit.rarity as ArmyUnit["rarity"])
          ? (unit.rarity as ArmyUnit["rarity"])
          : "Common",
...
function sanitizeProgress(parsed: Partial<PlayerProgress>): PlayerProgress {
  const partnerLevel = toInt(
    parsed.partnerLevel,
    DEFAULT_PROGRESS.partnerLevel,
    1,
  );
  ...
  return {
    ...
    partnerLevel,
    partnerForm: FORMS.includes(parsed.partnerForm as PartnerForm)
      ? (parsed.partnerForm as PartnerForm)
      : DEFAULT_PROGRESS.partnerForm,
    ...
    buildingLevel: toInt(
      parsed.buildingLevel,
      DEFAULT_PROGRESS.buildingLevel,
      1,
    ),
...
export function applyGateRewards(
  ...
  updated.gateTier = Math.max(
    updated.gateTier,
    1 + Math.floor(updated.partnerLevel / 3),
  );
  updated.chipLevel = Math.max(
    updated.chipLevel,
    1 + Math.floor(updated.partnerLevel / 5),
  );
  updated.buildingLevel = Math.max(
    updated.buildingLevel,
    1 + Math.floor(updated.playerLevel / 4),
  );
