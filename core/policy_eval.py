...
DEFAULT_RULES: List[PolicyRule] = [
    ...
    PolicyRule(
        ...
        on_true=PolicyDecision(admit=False, score=1.0, reasons=["High-risk action is not reversible"], severity="S4"),
    ),
    PolicyRule(
        name="deny_critical_impact_action",
        condition="'critical' in context.get('impact_tags', [])",
        on_true=PolicyDecision(admit=False, score=1.0, reasons=["Action has 'critical' impact tag"], severity="S4"),
    ),
    ...
    PolicyRule(
        ...
        on_true=PolicyDecision(admit=False, score=0.95, reasons=["Action cost exceeds 95% threshold"], severity="S3"),
    ),
    PolicyRule(
        name="deny_unverified_source",
        condition="context.get('source_trust', 'untrusted') == 'untrusted'",
        on_true=PolicyDecision(admit=False, score=0.9, reasons=["Action source is untrusted"], severity="S3"),
    ),
    ...
    PolicyRule(
        ...
        on_true=PolicyDecision(admit=True, score=0.8, reasons=["High-risk action, proceed with caution (reversible)"], severity="S2"),
    ),
    PolicyRule(
        name="warn_moderate_cost",
        condition="context.get('cost_score', 0) > 0.7",
        on_true=PolicyDecision(admit=True, score=0.7, reasons=["Action has moderate cost"], severity="S2"),
    ),
    ...
    PolicyRule(
        name="default_allow",
        condition="True",
        on_true=PolicyDecision(admit=True, score=0.1, reasons=["Default policy: allow"], severity="S1"),
    ),
]
...
def evaluate_action(action: Dict[str, Any], context: Dict[str, Any], rules: List[PolicyRule] = DEFAULT_RULES) -> PolicyDecision:
    """
    ...
    return PolicyDecision(admit=False, score=1.0, reasons=["Fell through all rules; default deny"], severity="S4")
