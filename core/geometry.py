...
class GeometryLayer:
        ...
        try:
            ...
            for tdesc in cfg.get("transforms", []):
                ...
                for t in self.transforms:
                    if t.name == name:
                        t.parameters.update({k: v for k, v in params.items() if not k.startswith("_")})
                        break
        except Exception:
    ...
    def _apply_scale(self, value: float, params: Dict[str, Any], context: Dict[str, Any]) -> float:
    ...
    def _apply_project(self, value: Any, params: Dict[str, Any], context: Dict[str, Any]) -> Any:
        """
        ...
        if projection_mode == "identity":
            ...
            if isinstance(value, dict):
                return math.sqrt(sum(v**2 for v in value.values() if isinstance(v, (int, float))))
            return abs(value)
            ...
            try:
                if transform.transform_type == TransformType.NORMALIZE:
                    if isinstance(value, (int, float)):
                        transformed[field] = self._apply_normalize(value, transform.parameters)
                
                elif transform.transform_type == TransformType.SCALE:
                    if isinstance(value, (int, float)):
                        transformed[field] = self._apply_scale(value, transform.parameters, context)
                
                elif transform.transform_type == TransformType.CLAMP:
                    if isinstance(value, (int, float)):
                        transformed[field] = self._apply_clamp(value, transform.parameters)
                
                elif transform.transform_type == TransformType.TRANSLATE:
                    if isinstance(value, (int, float)):
                        transformed[field] = self._apply_translate(value, transform.parameters)
                
                elif transform.transform_type == TransformType.PROJECT:
                    transformed[field] = self._apply_project(value, transform.parameters, context)
                
                elif transform.transform_type == TransformType.ROTATE:
                    if isinstance(value, (int, float)):
                        transformed[field] = self._apply_rotate(value, transform.parameters)
