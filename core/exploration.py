...
class ExplorationLayer:
        ...
        for param, bound in self.bounds.items():
            ...
            if value is not None and isinstance(value, (int, float)):
                if value < bound.safe_min or value > bound.safe_max:
                    return (
                        False,
                        f"{param} = {value} outside safe bounds [{bound.safe_min}, {bound.safe_max}]",
                    )
    ...
    def record_exploration(
        self, strategy: str, success: bool, reward: float, reverted: bool = False
    ):
...
def record_exploration(
    strategy: str, success: bool, reward: float, reverted: bool = False
):
