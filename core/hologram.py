...
LEVELS = [
    {"level": 0, "name": "EGG", "xp_min": 0, "label": "LV0 . EGG"},
    {"level": 1, "name": "ROOKIE", "xp_min": 5, "label": "LV1 . ROOKIE"},
    {"level": 2, "name": "CHAMPION", "xp_min": 20, "label": "LV2 . CHAMPION"},
    {"level": 3, "name": "ULTIMATE", "xp_min": 60, "label": "LV3 . ULTIMATE"},
    {"level": 4, "name": "MEGA", "xp_min": 150, "label": "LV4 . MEGA"},
    {"level": 5, "name": "BURST", "xp_min": 150, "label": "LV5 . BURST MODE"},
]
...
def compute_level(trials: Optional[List[Dict]] = None) -> Dict[str, Any]:
    ...
    # Determine level
    level = 0
    unlock_reason = "Need >=5 verified trials"

    if xp >= 5:
        level = 1
        unlock_reason = "Need >=20 verified trials AND >=5 per metric"

    if xp >= 20 and min_per_metric >= 5:
        level = 2
        unlock_reason = "Need >=60 verified trials AND >=20 in same context_tag"

    if xp >= 60 and max_context_tag >= 20:
        level = 3
        unlock_reason = "Need >=150 verified trials AND >=10 drift events"
...
def _why_failed_hint(trial: Dict) -> str:
    ...
    if expected_delta is not None:
        ...
        if after < before:
            hints.append(
                f"Metric worsened: {before} ? {after} (?={round(delta, 4)})"
            )
    ...
    if eval_ev.get("metric_before") is None and trial.get("metric_before") is None:
        hints.append("No baseline was captured -- comparison unreliable")
...
def suggest_drift_correction(metric_name: str, context: str) -> Optional[Dict]:
    ...
    # Build suggestion (not created yet -- operator must confirm)
    ...
    suggestion = {
        ...
        "notes": f"Auto-suggested: drift {round(drift['drift_magnitude'], 2)}? detected. Propose {direction} correction.",
        "drift_data": drift,
    }
...
def create_burst_batch(
    ...
    Requires: Burst Mode enabled, 3-10 trials, budget set.
    ...
    if len(trials_specs) < 3 or len(trials_specs) > max_parallel:
        return {"ok": False, "error": f"Burst batch requires 3-{max_parallel} trials"}
