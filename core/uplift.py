...
class UpliftLayer:
    ...
    def update_maturity(
        self, success: bool, confidence: float = 0.0, rolled_back: bool = False
    ):
        ...
        if total > 0:
            self.maturity.average_confidence = (
                self.maturity.average_confidence * (total - 1) + confidence
            ) / total
    ...
    def check_unlock_conditions(
        ...
        if "successful_actions" in conditions:
            if self.maturity.successful_actions < conditions["successful_actions"]:
                return (
                    False,
                    f"Need {conditions['successful_actions']} successful actions (have {self.maturity.successful_actions})",
                )
        ...
        if "success_rate" in conditions:
            if self.maturity.success_rate() < conditions["success_rate"]:
                return (
                    False,
                    f"Need {conditions['success_rate']*100}% success rate (have {self.maturity.success_rate()*100:.1f}%)",
                )
        ...
        if "uptime_hours" in conditions:
            if self.maturity.uptime_hours < conditions["uptime_hours"]:
                return (
                    False,
                    f"Need {conditions['uptime_hours']}h uptime (have {self.maturity.uptime_hours:.1f}h)",
                )
        ...
        if "rollback_count_max" in conditions:
            if self.maturity.rollback_count > conditions["rollback_count_max"]:
                return (
                    False,
                    f"Too many rollbacks ({self.maturity.rollback_count} > {conditions['rollback_count_max']})",
                )
        ...
        if "average_confidence" in conditions:
            if self.maturity.average_confidence < conditions["average_confidence"]:
                return (
                    False,
                    f"Need {conditions['average_confidence']} avg confidence (have {self.maturity.average_confidence:.2f})",
                )
        ...
        if newly_unlocked:
            return LayerResult(
                ...
                metadata={
                    ...
                    "total_unlocked": sum(
                        1 for c in self.capabilities.values() if c.unlocked
                    ),
                    "maturity": {
