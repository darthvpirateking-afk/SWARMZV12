...
class StabilizationLayer:
    ...
    def _load_config(self) -> None:
        ...
        try:
            ...
            self._default_threshold_crossings = osc.get("threshold_crossings_to_flag", 3)
    ...
    def apply_damping(self, current_value: float, target_value: float, damping_factor: float = 0.5) -> float:
        """
    ...
    def apply_hysteresis(self, value: float, threshold: float, last_state: bool) -> bool:
        """
    ...
    def evaluate(self, action: Dict[str, Any], context: Dict[str, Any]) -> LayerResult:
        ...
        oscillating_metrics = [
            name for name, detector in self.oscillation_detectors.items()
...
def apply_damping(current_value: float, target_value: float, damping_factor: float = 0.5) -> float:
