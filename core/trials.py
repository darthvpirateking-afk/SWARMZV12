...
def get_trial(trial_id: str) -> Optional[Dict[str, Any]]:
    """Get a single trial by id. Scans JSONL directly to avoid building the full
    deduplication dict that load_all_trials() constructs; last entry wins."""
    rows = _read_jsonl(_TRIALS_FILE)
    result = None
    for r in rows:
...
def inbox_counts() -> Dict[str, int]:
    """Quick counts for all three tabs (single-pass)."""
    trials = load_all_trials()
    pending = 0
    needs_review = 0
    completed = 0
