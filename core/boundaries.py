...
class BoundariesLayer:
                ...
                if any(r.name == name for r in self.rules):
                    ...
                    for r in self.rules:
                        if r.name == name:
                            ...
                            r.requires_interface = rdesc.get("requires_interface", r.requires_interface)
                    ...
                    try:
                        self.add_rule(BoundaryRule(
                            ...
                            requires_interface=rdesc.get("requires_interface", False),
    ...
    def _find_rule(self, from_domain: DomainType, to_domain: DomainType) -> Optional[BoundaryRule]:
    ...
    def check_boundary_crossing(
        ...
        if from_domain == to_domain:
            return LayerResult(
                ...
                metadata={"from_domain": from_domain.value, "to_domain": to_domain.value},
                timestamp=time.time(),
            )
...
def check_boundary_crossing(action: Dict[str, Any], context: Dict[str, Any]) -> LayerResult:
