...
class MissionEngine:
    ...
    def _space_worker(self, params: Dict[str, Any]) -> LayerResult:
        ...
        return self.space_ops.execute_telemetry_check(
            params.get("target", "L4_libration_point"), params
        )
    ...
    def execute(self, mission_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        telemetry.log_action(
            "INFO",
            "MissionEngine",
            f"Initiating mission {mission_id}",
        ...
        if cognition == CognitiveState.CRITICAL and has_domain_tasks:
            ...
            telemetry.log_action(
                "CRITICAL", "MissionEngine", msg, mission_id=mission_id
            )
            return {
                "ok": False,
                "error": msg,
                "cognition": reflector.get_cognition_summary(),
            }
        ...
        for task_id in execution_order:
            ...
            try:
                ...
                worker = self._workers.get(
                    cap_id, lambda p: self._internal_worker(cap_id, p)
                )
