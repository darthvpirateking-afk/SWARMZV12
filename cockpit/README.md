...
2. Start dev server (proxies `/v1/canonical/*` to backend at `http://localhost:8000`):
   ```
   npm run dev
   ```
...
4. Notes:
   - Ensure your backend (nexusmon server) is running on `http://localhost:8000` and exposes `/v1/canonical/cockpit/hologram/ws` and `/v1/canonical/cockpit/hologram/snapshot/latest`.
