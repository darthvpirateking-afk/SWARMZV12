"""Auto-generated manifest validation test for plugin-helper1.manifest.json."""
...
def test_manifest_plugin_helper1_manifest() -> None:
    repo_root = Path(__file__).resolve().parent.parent
    schema = json.loads((repo_root / "schemas" / "agent-manifest.v1.json").read_text(encoding="utf-8-sig"))
    manifest = json.loads((repo_root / "config" / "manifests" / "plugin-helper1.manifest.json").read_text(encoding="utf-8-sig"))
