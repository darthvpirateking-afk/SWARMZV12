...
def test_snapshot_file_copies_content(rollback_engine: RollbackEngine, test_workspace: Path):
...
def test_rollback_restores_file_content(rollback_engine: RollbackEngine, test_workspace: Path):
...
def test_rollback_recreates_deleted_file(rollback_engine: RollbackEngine, test_workspace: Path):
...
def test_cannot_commit_after_rollback(rollback_engine: RollbackEngine, test_workspace: Path):
...
def test_transaction_log_is_written_correctly(rollback_engine: RollbackEngine, test_workspace: Path):
    ...
    events = [json.loads(line) for line in log_content_before_rollback.strip().split('\n')]
    ...
    final_events = [json.loads(line) for line in transaction.log_content_before_delete.strip().split('\n')]
