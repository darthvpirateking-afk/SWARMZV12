...
def _make_trial(
    ...
    return {
        ...
        "checked_at": (created + timedelta(seconds=350)).isoformat() if checked else None,
...
def _make_trials(n, metric_names=None, tags=None, survived_ratio=1.0, context="onboarding"):
    """Generate n trials."""
...
class TestLevel:
    ...
    def test_level_3_blocked_insufficient_context_tag(self):
        ...
        trials = _make_trials(60, metric_names=["a", "b"],
                              tags=["t1", "t2", "t3", "t4", "t5"])
        state = compute_level(trials)
...
class TestCurrencies:
    ...
    def test_novelty_zero_no_recent(self):
        ...
        for i in range(5):
            trials.append(_make_trial(
                f"t{i}", checked=True, created_days_ago=60
            ))
        cur = compute_power_currencies(trials)
...
class TestEffects:
    ...
    def test_effects_survived_trial(self):
        from core.hologram import compute_effects
        trial = _make_trial("t1", survived=True, metric_before=0.5, metric_after=0.6, expected_delta=0.1)
    ...
    def test_effects_failed_trial_flicker(self):
        from core.hologram import compute_effects
        trial = _make_trial("t1", survived=False, metric_before=0.5, metric_after=0.4, expected_delta=0.1)
        effects = compute_effects(trial, 1)
...
class TestWhyFailed:
    def test_shortfall_hint(self):
        ...
        trial = _make_trial(survived=False, metric_before=0.5, metric_after=0.52, expected_delta=0.1)
        hint = _why_failed_hint(trial)
        assert "short" in hint.lower() or "shortfall" in hint.lower()

    def test_worsened_hint(self):
        from core.hologram import _why_failed_hint
        trial = _make_trial(survived=False, metric_before=0.5, metric_after=0.4, expected_delta=None)
        hint = _why_failed_hint(trial)
...
class TestDrift:
    ...
    def test_drift_detection_with_data(self):
        ...
        with patch("core.hologram.load_all_trials", return_value=all_trials), \
...
class TestBurstMode:
    ...
    def test_burst_toggle_requires_lv4(self):
        ...
        with patch("core.hologram.compute_level", return_value={
            "level": 1, "currencies": {"reversibility": 0.5}
        }):
    ...
    def test_burst_toggle_requires_reversibility(self):
        from core.hologram import toggle_burst_mode
        with patch("core.hologram.compute_level", return_value={
            "level": 4, "currencies": {"reversibility": 0.0}
