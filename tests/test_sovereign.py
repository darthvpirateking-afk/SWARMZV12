...
def test_evaluate_condition_simple_equality():
    ...
    assert sovereign._evaluate_condition("action_type == 'critical'", {"action_type": "critical"})
    assert not sovereign._evaluate_condition("action_type == 'critical'", {"action_type": "normal"})
...
def test_evaluate_condition_boolean():
    ...
    assert sovereign._evaluate_condition("security_violation == True", {"security_violation": True})
    assert not sovereign._evaluate_condition("security_violation == True", {"security_violation": False})
...
def test_evaluate_condition_inequality():
    ...
    assert sovereign._evaluate_condition("budget_remaining <= 0", {"budget_remaining": 0})
    assert sovereign._evaluate_condition("budget_remaining <= 0", {"budget_remaining": -10})
    assert not sovereign._evaluate_condition("budget_remaining <= 0", {"budget_remaining": 10})
