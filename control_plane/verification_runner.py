#!/usr/bin/env python3
"""
verification_runner.py – Swarmz-native Verification Runner Service.

A durable, event-driven service that:

* Consumes ``ACTION_SELECTED`` events and **idempotently** enqueues
  verification jobs.
* Uses **Postgres** as source of truth with a persistent deadline
  scheduler driven by **LISTEN/NOTIFY** (no polling loops).
* Claims due work via **lease + SKIP LOCKED** (multi-runner safe).
* Evaluates **verification metric deltas** from ``state.jsonl``
  using a baseline byte offset.
* Emits ``VERIFY_PASSED`` / ``VERIFY_FAILED`` / ``EXPIRED`` with
  full provenance and **exactly-once final outcome** per job.
* Triggers **rollback ACT** on ``VERIFY_FAILED`` with
  **exactly-once rollback trigger**.
* Correlates rollback results and persists them.
* Logs outcomes to **decision_log** (config_hash-linked) and emits
  **calibration hooks**.
* Dead-letters after max attempts.
* Emits SwarmHealth heartbeats and core metrics.
    ...
    def check_pending(self):
        ...
        for job in claimed:
            ...
            if job["attempts"] > max_att:
                ...
                continue
            self._evaluate_and_finalize(job)

    def _evaluate_and_finalize(self, job: dict):
        """Run delta evaluation and emit outcome."""
        result = self._evaluator.evaluate(job)
        passed = result["passed"]

        if passed:
            outcome_status = STATUS_PASSED
            event_type = "VERIFY_PASSED"
        else:
            outcome_status = STATUS_FAILED
        ...
        finalized = self._jobs.finalize(
            job["job_id"], self._runner_id,
            outcome_status, event_type,
            error=result.get("failure_reason"))

        if not finalized:
            return  # Another runner already finalized
        ...
        if passed:
            ...
            print(f"[verify] FAILED: {job['action_id']} "
                  f"– {result.get('failure_reason')}")
            self._do_rollback(job)

    def _do_dead_letter(self, job: dict, reason: str):
        """Dead-letter a job after max attempts."""
        success = self._jobs.dead_letter(
            job["job_id"], self._runner_id, reason)
        if not success:
            return
        ...
        self._bus.publish("DEADLETTERED", payload)
        ...
        print(f"[verify] DEADLETTERED: {job['action_id']} – {reason}")
