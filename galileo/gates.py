...
def gate_g1_specific(hypothesis: Dict) -> Tuple[bool, str]:
    ...
    has_metric = bool(
        re.search(
            r"[a-z]+_[a-z]+|[A-Z][a-z]+|accuracy|precision|recall|auc|rmse|mae",
            pred_text,
        )
    )
...
def gate_g2_testable_locally(hypothesis: Dict) -> Tuple[bool, str]:
    ...
    has_synthetic = any(
        indicator in protocol_str
        for indicator in ["synthetic", "simulation", "monte carlo", "generated", "mock"]
    )
...
def gate_g4_novelty(
    ...
    if "closest_known" not in novelty_anchor or "difference" not in novelty_anchor:
        return (
            False,
            "novelty_anchor missing required fields: closest_known, difference",
        )
    ...
    if similarity_score >= threshold:
        return (
            False,
            f"Similarity {similarity_score:.2%} exceeds threshold {threshold:.0%}",
        )

    return (
        True,
        f"Novel (similarity {similarity_score:.2%}, below threshold {threshold:.0%})",
    )


def apply_gates(
    hypothesis: Dict, similarity_score: float = 0.0, threshold: float = 0.82
    ...
    gates = [
        ...
        (
            "G4_NOVELTY",
            lambda h: gate_g4_novelty(
                h, similarity_score=similarity_score, threshold=threshold
            ),
        ),
    ]
