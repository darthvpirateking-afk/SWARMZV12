...
from .storage import (
    init_storage, load_hypotheses, load_priors, load_domain_pack,
    save_hypothesis, save_experiment, save_score, save_run_record
)
from .prompts import (
...
def run_galileo(
    ...
    run_record = {
        ...
        'domain': domain,
        'seed': seed,
        'inputs_hash': stableStringify({'domain': domain, 'seed': seed, 'n_hypotheses': n_hypotheses}),
    ...
    generated = _generate_hypotheses(
        domain, n_hypotheses, hypothesis_ids, domain_pack, seed,
        llm_client=llm_client, use_synthetic=use_synthetic
    )
    hypotheses.extend(generated)
    
    # --- STAGE 2: CRITIQUE each ---
    for hyp in hypotheses:
        critique = _critique_hypothesis(hyp, domain, llm_client=llm_client, use_synthetic=use_synthetic)
        hyp['critique'] = critique
    
    # --- STAGE 3: SPECIFY TEST (LLM experimentalist) ---
    for hyp in hypotheses:
        experiment = _specify_experiment(hyp, domain, seed, llm_client=llm_client, use_synthetic=use_synthetic)
    ...
    for hyp in hypotheses:
        ...
        # Score
        scores = score_hypothesis(hyp, similarity_score=similarity, gate_failures=gates_failed)
...
"""
    ...
    instructions += f"See experiments.json for full protocol details.\n"
    instructions += f"All experiments are LOCAL ONLY (no external API calls).\n"
    instructions += f"Operator-gated execution: ensure review before running.\n"
...
def _generate_hypotheses(
    domain: str, n: int, hypothesis_ids: Dict, domain_pack: Dict,
    seed: int, llm_client=None, use_synthetic=False
) -> List[Dict]:
...
def _critique_hypothesis(hyp: Dict, domain: str, llm_client=None, use_synthetic=False) -> Dict:
...
def _specify_experiment(hyp: Dict, domain: str, seed: int, llm_client=None, use_synthetic=False) -> Dict:
...
def _synthetic_hypotheses(domain: str, n: int, hypothesis_ids: Dict) -> List[Dict]:
    ...
    for idx in range(n):
        hyp = {
            ...
            'novelty_anchor': {'closest_known': 'Generic baseline', 'difference': 'Specific to this domain'},
