...
def score_test_cost(hypothesis: Dict) -> float:
    ...
    is_synthetic = any(
        kw in protocol_str for kw in ["synthetic", "simulation", "mock", "local"]
    )
...
def score_hypothesis(
    hypothesis: Dict, similarity_score: float = 0.0, gate_failures: List[str] = None
) -> Dict[str, float]:
    """
...
def should_accept(
    ...
    if avg_score < novelty_threshold:
        return (
            False,
            f"Average score {avg_score:.2f} below threshold {novelty_threshold}",
        )
