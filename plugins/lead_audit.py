...
def calculate_value_score(value: str) -> float:
    ...
    try:
        ...
        value_clean = (
            value.replace("$", "").replace(",", "").replace("â‚¬", "").replace("Â£", "")
        )
...
def score_lead(lead: Dict[str, Any]) -> Dict[str, Any]:
    ...
    composite_score = engagement_score * 0.5 + value_score * 0.3 + recency_score * 0.2
...
def audit_leads(csv_content: str) -> Dict[str, Any]:
    ...
    return {"leads": scored_leads, "summary": summary, "recommendation": recommendation}
...
def register(executor):
    ...
    def score_single_lead(
        name: str, last_contact: str, value: str, engagement: str
    ) -> Dict[str, Any]:
        """Score a single lead."""
        lead = {
            "name": name,
