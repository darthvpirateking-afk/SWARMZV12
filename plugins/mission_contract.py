...
def validate_mission_contract(mission: Dict[str, Any]) -> Dict[str, Any]:
    ...
    missing_fields = [field for field in REQUIRED_TOP_LEVEL_FIELDS if field not in mission]
    if missing_fields:
    ...
    if "reality_signal" not in mission:
        errors.append("Mission must include external reality_signal (payment_received, user_reply, etc.)")
    ...
    if not ethics:
        ...
        if missing_ethics:
            warnings.append(f"Consider adding ethics constraints: {', '.join(missing_ethics)}")
    ...
    if not constraints:
        warnings.append("Consider specifying operational constraints (budget, resources, etc.)")
...
def check_law_compliance(mission: Dict[str, Any], law_name: str) -> Dict[str, Any]:
    ...
    law_checks = {
        ...
        "underspecification": len([f for f in REQUIRED_TOP_LEVEL_FIELDS if f in mission]) == len(REQUIRED_TOP_LEVEL_FIELDS),
        ...
        "measurable_success": "success_metrics" in mission and mission["success_metrics"],
        ...
        "channel_specificity": "channels" in mission and len(mission.get("channels", [])) > 0,
...
def register(executor):
    ...
    # Register tasks
    executor.register_task("mission_contract_validate", validate_contract, {
        "description": "Validate mission contract against 12 laws",
        "params": {"mission": "dict"},
        "category": "mission_contract"
    })
    
    executor.register_task("mission_contract_check_law", check_law, {
        "description": "Check compliance with specific law",
        "params": {"mission": "dict", "law_name": "string"},
        "category": "mission_contract"
    })
    
    executor.register_task("mission_contract_list_laws", list_laws, {
        "description": "List all 12 laws",
        "params": {},
        "category": "mission_contract"
    })
    
    executor.register_task("mission_contract_required_fields", list_required_fields, {
