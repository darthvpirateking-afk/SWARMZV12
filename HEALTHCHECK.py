...
class HealthChecker:
    ...
    def check_core_system(self) -> bool:
        ...
        try:
            ...
            # Test initialization
            if not self.check(
                "SwarmzCore initialization", lambda: SwarmzCore() is not None
            ):
                return False
            ...
            # Test audit log
            self.check(
                "Audit log access", lambda: isinstance(swarmz.get_audit_log(), list)
            )
    ...
    def check_builtin_tasks(self) -> bool:
        ...
        try:
            ...
            self.check(
                "System info task",
                lambda: isinstance(result, dict) and "platform" in result,
            )
    ...
    def check_plugins(self) -> bool:
        ...
        try:
            ...
            if os.path.exists(plugin_path):
                ...
                self.check(
                    "Load filesystem plugin", lambda: plugin_name == "filesystem"
                )

                # Test a filesystem task
                tasks = swarmz.list_capabilities()
                self.check(
                    "Filesystem tasks registered",
                    lambda: any("fs_" in task for task in tasks),
                )
            else:
            ...
            if os.path.exists(plugin_path):
                plugin_name = swarmz.load_plugin(plugin_path)
                self.check(
                    "Load dataprocessing plugin",
                    lambda: plugin_name == "dataprocessing",
                )

                # Test a data processing task
                tasks = swarmz.list_capabilities()
                self.check(
                    "Data processing tasks registered",
                    lambda: any("data_" in task for task in tasks),
                )
            else:
    ...
    def check_configuration(self) -> bool:
        ...
        if os.path.exists("config.json"):
            ...
            try:
                ...
                self.check(
                    "Config has operator_sovereignty",
                    lambda: "operator_sovereignty" in config,
                )
    ...
    def run_test_suite(self) -> bool:
        ...
        try:
            ...
            passed = result.returncode == 0 and (
                "OK" in result.stdout or "Success rate: 100" in result.stdout
            )
    ...
    def check_file_structure(self) -> bool:
        ...
        required_files = ["swarmz.py", "swarmz_cli.py", "README.md"]
    ...
    def generate_report(self) -> Dict:
        ...
        success_rate = (
            (passed_count / self.total_checks * 100) if self.total_checks > 0 else 0
        )
